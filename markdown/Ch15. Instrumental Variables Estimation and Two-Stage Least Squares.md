...
**15.1** TODO: objective for "Motivation: Omitted Variables in a Simple Regression Model"

**15.2** TODO: objective for "IV Estimation of the Multiple Regression Model"

**15.3** TODO: objective for "Two-Stage Least Squares"

**15.4** TODO: objective for "IV Solutions to Errors-in-Variables Problems"

**15.5** TODO: objective for "Testing for Endogeneity and Testing Overidentifying Restrictions"

**15.6** TODO: objective for "2SLS with Heteroskedasticity"

**15.7** TODO: objective for "Applying 2SLS to Time Series Equations"

**15.8** TODO: objective for "Applying 2SLS to Pooled Cross Sections and Panel Data"
...
axes[0].plot(
    z,
    first_stage.params.iloc[0] + first_stage.params.iloc[1] * z,
    "r-",
    linewidth=2,
    label="First Stage",
)
...
axes[0].text(
    0.05,
    0.95,
    f"Slope = {first_stage.params.iloc[1]:.3f}\nt-stat = {first_stage.tvalues.iloc[1]:.2f}",
...
axes[1].plot(
    z,
    reduced_form.params.iloc[0] + reduced_form.params.iloc[1] * z,
    "g-",
    linewidth=2,
    label="Reduced Form",
)
...
axes[1].text(
    0.05,
    0.95,
    f"IV = Reduced Form / First Stage\n= {reduced_form.params.iloc[1]:.4f} / {first_stage.params.iloc[1]:.3f}\n= {b_iv_manual:.4f}",
...
### Example 15.2: Returns to Education with Exogenous Controls

We extend the §15.1 wage equation by adding labor-market experience and its square as exogenous controls, while continuing to instrument `educ` with father's education. Including controls is essential when, for instance, work experience is itself correlated with education (more-educated workers entered the labor market later). The IV estimate now answers: *holding experience fixed, what is the effect of an extra year of schooling on log wages?*

```{code-cell} ipython3
mroz = wool.data("mroz")
mroz_working = mroz.dropna(subset=["lwage"])

# OLS baseline (treats educ as exogenous)
ols = smf.ols(
    "np.log(wage) ~ educ + exper + I(exper**2)", data=mroz_working,
).fit()

# 2SLS with fatheduc as instrument for educ; exper and exper^2 are exogenous.
iv = IV2SLS.from_formula(
    "np.log(wage) ~ 1 + exper + I(exper**2) + [educ ~ fatheduc]",
    data=mroz_working,
).fit(cov_type="unadjusted", debiased=True)

# First stage (the partial relevance of fatheduc after partialling out controls).
first_stage = smf.ols("educ ~ fatheduc + exper + I(exper**2)", data=mroz_working).fit()

comparison = pd.DataFrame(
    {
        "OLS": [
            ols.params["educ"], ols.bse["educ"],
            ols.params["exper"], ols.params["I(exper ** 2)"],
        ],
        "2SLS": [
            iv.params["educ"], iv.std_errors["educ"],
            iv.params["exper"], iv.params["I(exper ** 2)"],
        ],
    },
    index=["β̂_educ", "SE(β̂_educ)", "β̂_exper", "β̂_exper²"],
)
display(comparison.round(4))
print(f"\nFirst-stage t on fatheduc (after partialling out exper, exper²): "
      f"{first_stage.tvalues['fatheduc']:.2f}")
print("(rule of thumb: |t|>3.3 ≈ F>10 ≈ strong instrument)")
```

The 2SLS slope on `educ` is somewhat smaller than OLS in this sample (about seven percent per year rather than nearly eleven percent) and comes with a standard error roughly two and a half times as large, the typical tradeoff when we instrument an endogenous regressor. The shift is consistent with father's education being correlated with omitted family-background variables that also raise wages; once we isolate the variation in own-education driven by father's education alone, the estimated return falls. The experience coefficients change only slightly between OLS and 2SLS, illustrating that the IV machinery acts on the endogenous variable while leaving the exogenous controls' interpretation intact. The first-stage t-statistic on `fatheduc` after partialling out experience is the relevance check Wooldridge emphasizes: a value comfortably above three signals a strong instrument and protects the 2SLS standard errors from blowing up.
