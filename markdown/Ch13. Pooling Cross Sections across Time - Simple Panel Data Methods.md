...
**13.1** TODO: objective for "Pooling Independent Cross Sections across Time"

**13.2** TODO: objective for "Policy Analysis with Pooled Cross Sections"

**13.3** TODO: objective for "Two-Period Panel Data Analysis"

**13.4** TODO: objective for "Policy Analysis with Two-Period Panel Data"

**13.5** TODO: objective for "Differencing with More Than Two Time Periods"
...
### Example 13.5: First-Differencing the Crime-Unemployment Relationship

The `crime2` dataset records crime rates and unemployment for 46 U.S. cities in 1982 and 1987. A pooled-OLS regression of `crmrte` on `unem` confounds cross-city differences (some cities are simply higher-crime than others) with within-city changes. First-differencing wipes out the city effect $a_i$ and identifies the effect of *changes* in unemployment on *changes* in crime.

```{code-cell} ipython3
crime2 = wool.data("crime2")

# Wide format: one row per city with year-82 and year-87 columns.
crime_wide = (
    crime2.assign(city=(crime2.index // 2).astype(int))
    .pivot(index="city", columns="year", values=["crmrte", "unem"])
    .dropna()
)
crime_wide.columns = [f"{var}_{yr}" for var, yr in crime_wide.columns]

# First differences
crime_wide["dcrmrte"] = crime_wide["crmrte_87"] - crime_wide["crmrte_82"]
crime_wide["dunem"] = crime_wide["unem_87"] - crime_wide["unem_82"]

# Pooled OLS (ignores the city effect) vs first-difference estimator.
pooled = smf.ols("crmrte ~ unem", data=crime2).fit()
fd = smf.ols("dcrmrte ~ dunem", data=crime_wide).fit()

comparison = pd.DataFrame(
    {
        "Estimator": ["Pooled OLS (crmrte on unem)", "First differences (Δcrmrte on Δunem)"],
        "β̂_unem": [pooled.params["unem"], fd.params["dunem"]],
        "SE": [pooled.bse["unem"], fd.bse["dunem"]],
        "t-stat": [pooled.tvalues["unem"], fd.tvalues["dunem"]],
        "R²": [pooled.rsquared, fd.rsquared],
    },
)
display(comparison.round(3))
```

Pooled OLS finds no association: unemployment varies across cities for many reasons (industrial mix, demographics) that are themselves correlated with crime through $a_i$. First-differencing differences out $a_i$ and recovers a positive within-city slope: when a given city's unemployment rises, its crime rate rises with it. The first-difference estimator is unbiased under MLR.4 applied to $\Delta u_i$, which is a weaker condition than requiring $a_i$ to be unrelated to unemployment.
...
### Example 13.6: The Kiel Incinerator and Housing Prices

Wooldridge's `kielmc` dataset records housing sales near North Andover, MA in 1978 (pre-announcement) and 1981 (after construction of a garbage incinerator was announced). Houses with `nearinc == 1` are the treatment group; the rest are the control group. The DiD estimator answers: *did prices of houses near the future incinerator fall more sharply than prices elsewhere between 1978 and 1981?*

```{code-cell} ipython3
kielmc = wool.data("kielmc")

# Standard interaction specification on the level rprice.
did_levels = smf.ols("rprice ~ y81 * nearinc", data=kielmc).fit()

# Same in logs, which removes much of the heteroskedasticity in housing prices.
did_logs = smf.ols("np.log(rprice) ~ y81 * nearinc", data=kielmc).fit()

# Report the four sample means that determine the DiD estimate.
cell_means = (
    kielmc.assign(group=np.where(kielmc["nearinc"] == 1, "near", "far"),
                  period=np.where(kielmc["y81"] == 1, "1981", "1978"))
    .groupby(["group", "period"], observed=True)["rprice"]
    .mean()
    .unstack()
)
print("Sample means of real price (1978$):")
display(cell_means.round(0))
manual_did = (cell_means.loc["near", "1981"] - cell_means.loc["near", "1978"]) - (
    cell_means.loc["far", "1981"] - cell_means.loc["far", "1978"]
)
print(f"\nManual DiD (delta near minus delta far) = {manual_did:,.0f} dollars")
print(f"Regression DiD (y81:nearinc coefficient) = {did_levels.params['y81:nearinc']:,.0f} dollars")
print(f"Regression DiD in logs (≈ percentage change) = {did_logs.params['y81:nearinc']:.3f}")
```

The manual DiD computation from the four cell means equals the regression coefficient on the `y81:nearinc` interaction, by construction. The point estimate is negative in both level and log form: houses near the incinerator lost roughly twelve thousand dollars (about six log points) in value relative to comparable houses farther away. The standard error on the unconditional DiD is large enough that the 95% confidence interval just covers zero, which is why Wooldridge follows this example by adding controls for house age, size, and rooms; once those soak up residual variation, the same DiD coefficient becomes precisely estimated. The parallel-trends assumption is plausible here because the two areas track each other in the 1978 cross-section, and the treatment was driven by exogenous siting decisions rather than by 1978 price levels.
