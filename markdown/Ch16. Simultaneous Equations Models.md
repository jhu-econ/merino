...
### Example 16.1: Simulating a Supply-Demand System

To see the difference between structural and reduced forms in numbers, we simulate the system above with known parameters, then estimate both the structural and reduced forms on the simulated equilibrium data. Because we know the true parameters, we can check whether OLS on the structural equations recovers them (it will not) and whether the reduced-form regressions are clean (they will be).

```{code-cell} ipython3
rng = np.random.default_rng(2026)
n = 800

# Exogenous demand shifter (income) and supply shifter (weather).
income = rng.normal(0, 1, size=n)
weather = rng.normal(0, 1, size=n)
u_d = rng.normal(0, 1, size=n)
u_s = rng.normal(0, 1, size=n)

# True structural parameters
alpha = {"intercept": 5.0, "price": -1.5, "income": 1.0}  # demand: Q = α0 + α1 P + α2 I + u_d
beta = {"intercept": 1.0, "price": 0.8, "weather": 1.2}   # supply: Q = β0 + β1 P + β2 W + u_s

# Reduced form derived from setting D = S and solving for price.
denom = beta["price"] - alpha["price"]
price = (
    (alpha["intercept"] - beta["intercept"])
    + alpha["income"] * income
    - beta["weather"] * weather
    + (u_d - u_s)
) / denom
quantity = alpha["intercept"] + alpha["price"] * price + alpha["income"] * income + u_d

sim = pd.DataFrame({"price": price, "quantity": quantity, "income": income, "weather": weather})

# Structural OLS (biased: price is endogenous in both equations).
demand_ols = smf.ols("quantity ~ price + income", data=sim).fit()
supply_ols = smf.ols("quantity ~ price + weather", data=sim).fit()

# Reduced form: regress each endogenous variable on ALL exogenous variables.
rf_price = smf.ols("price ~ income + weather", data=sim).fit()
rf_quantity = smf.ols("quantity ~ income + weather", data=sim).fit()

summary = pd.DataFrame(
    {
        "True α₁ (price in demand)": [alpha["price"]],
        "OLS demand β̂_price": [demand_ols.params["price"]],
        "True β₁ (price in supply)": [beta["price"]],
        "OLS supply β̂_price": [supply_ols.params["price"]],
    },
).T.rename(columns={0: "value"})
display(summary.round(3))

print("Reduced-form regressions (consistent: all RHS exogenous):")
display(
    pd.DataFrame(
        {
            "price ~": [rf_price.params["income"], rf_price.params["weather"]],
            "quantity ~": [rf_quantity.params["income"], rf_quantity.params["weather"]],
        },
        index=["income", "weather"],
    ).round(3),
)
```

The structural OLS estimates miss the true parameters badly: applying OLS to the demand equation gives a slope on `price` that is far from $-1.5$, and similarly for supply. The reason is the same as in Chapter 15 (`price` is correlated with the structural error), but here the correlation arises from market equilibrium rather than from omitted variables. The reduced-form regressions are unproblematic: their right-hand sides contain only exogenous variables (`income` and `weather`), so OLS is consistent and the four reduced-form coefficients identify combinations of the underlying structural parameters. That is the bridge §16.3 will use to back out structural parameters via 2SLS.
...
### Example 16.5: Estimating the Mroz Labor-Supply System

We estimate the labor-supply equation by 2SLS, instrumenting `log(wage)` with `exper` and `exper²`, which are excluded from the supply equation but enter the wage equation. The order condition delivers two excluded exogenous variables for one endogenous regressor, so the equation is overidentified, which means we can also run a Sargan-style overidentification test.

```{code-cell} ipython3
mroz = wool.data("mroz")
labor = mroz.dropna(subset=["lwage"]).copy()
labor["expersq"] = labor["exper"] ** 2

# OLS treats log(wage) as exogenous (biased under simultaneity).
ols_labor = smf.ols("hours ~ lwage + educ + age + kidslt6 + nwifeinc", data=labor).fit()

# 2SLS: instrument log(wage) with exper and exper^2.
iv_labor = IV2SLS.from_formula(
    "hours ~ 1 + educ + age + kidslt6 + nwifeinc + [lwage ~ exper + expersq]",
    data=labor,
).fit(cov_type="robust")

# First stage to confirm relevance.
first_stage = smf.ols(
    "lwage ~ exper + expersq + educ + age + kidslt6 + nwifeinc", data=labor,
).fit()
joint_F = first_stage.f_test("exper = 0, expersq = 0").fvalue

comparison = pd.DataFrame(
    {
        "OLS β̂": [ols_labor.params[k] for k in ["lwage", "educ", "kidslt6"]],
        "OLS SE": [ols_labor.bse[k] for k in ["lwage", "educ", "kidslt6"]],
        "2SLS β̂": [iv_labor.params[k] for k in ["lwage", "educ", "kidslt6"]],
        "2SLS SE": [iv_labor.std_errors[k] for k in ["lwage", "educ", "kidslt6"]],
    },
    index=["lwage (endogenous)", "educ", "kidslt6"],
)
display(comparison.round(2))
print(f"\nFirst-stage joint F on excluded instruments (exper, exper²): {float(joint_F):.2f}")
print(f"Sargan overid p-value (H₀: instruments valid): {iv_labor.sargan.pval:.3f}")
```

The OLS coefficient on `lwage` is near zero with a wide standard error; under simultaneity it does not consistently estimate the structural elasticity. The 2SLS coefficient is large and positive (women facing higher market wages work more hours, conditional on the controls), but the standard error is also enormous, and the first-stage F on the excluded instruments hovers near the conventional weak-instrument threshold of ten. This is what *partial identification with weak instruments* looks like in practice: the point estimate moves dramatically but the confidence interval is so wide that we cannot pin down the structural elasticity. The Sargan p-value does fail to reject the overidentifying restrictions, meaning the two excluded instruments are mutually consistent; mutual consistency, however, does not guarantee strength. The honest reading is that `exper` and `exper²` are not powerful enough instruments for `lwage` on this sample, and credible inference would require stronger excluded variables.
