...
**11.1** TODO: objective for "Asymptotics with Time Series"

**11.2** TODO: objective for "The Nature of Highly Persistent Time Series"

**11.3** TODO: objective for "Differences of Highly Persistent Time Series"

**11.4** TODO: objective for "Regression with First Differences"
...
The asymptotic results above assume a homoskedastic error process: $\text{Var}(u_t \mid \mathbf{x}_t) = \sigma^2$, constant across time. Heteroskedasticity in time series regressions has the same OLS consequences as in cross sections (unbiased estimates, invalid conventional standard errors), but it can also take time-series-specific forms such as volatility clustering in financial data, or scale effects when both $y_t$ and $x_t$ trend upward. Two practical questions arise. Are the residuals homoskedastic? Standard Breusch-Pagan and White tests work on time series residuals provided we have first ruled out serial correlation (Chapter 12). If not, what do we do? Heteroskedasticity-robust standard errors (HC1/HC3) are the time-series analogue of Chapter 8; when serial correlation is also present, HAC standard errors (Chapter 12) combine both corrections.

```{code-cell} ipython3
from statsmodels.stats.diagnostic import het_breuschpagan, het_white

# Reuse the first-difference fertility model from §11.4.
fd_panel = fertil3[["gfr_diff1", "pe_diff1"]].dropna()
fd = smf.ols("gfr_diff1 ~ pe_diff1", data=fd_panel).fit()

bp_lm, bp_p, _, _ = het_breuschpagan(fd.resid, fd.model.exog)
white_lm, white_p, _, _ = het_white(fd.resid, fd.model.exog)

# Compare conventional vs heteroskedasticity-robust standard errors.
fd_hc1 = fd.get_robustcov_results(cov_type="HC1")

diagnostics = pd.DataFrame(
    {
        "Test": ["Breusch-Pagan", "White"],
        "LM stat": [bp_lm, white_lm],
        "p-value": [bp_p, white_p],
    },
)
...
se_table = pd.DataFrame(
    {
        "Conventional SE": fd.bse.values,
        "HC1 robust SE": fd_hc1.bse,
    },
    index=fd.bse.index,
)
...
If either test rejects, OLS remains unbiased and consistent but conventional standard errors are not consistent estimators of the sampling standard deviations. Switching to HC1 (or to HAC when serial correlation is also present) restores valid inference.

This chapter established essential principles for applying OLS to time series: the critical importance of stationarity and weak dependence for valid asymptotic inference, the distinct characteristics of highly persistent integrated I(1) processes including random walks, the use of first differencing as a systematic technique to induce stationarity and avoid spurious regression problems, and a brief tour of homoskedasticity diagnostics in the time-series setting. These foundations prepare for more advanced time series methods in subsequent chapters.
